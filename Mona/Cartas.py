...
class Carta(object):
    ...
    def __eq__(self, other):
        if isinstance(other, Carta):
            return other.rango==self.rango and other.palo==self.palo
...
class Mano(Mazo):
    ...
    def imprimir(self):
        ...
        if len(self.cartas) == 0:
            print("La mano esta vacia")
        else:
            print("La mano contiene: ")
            super().imprimir()
...
class ManoPersona(Mano):
    ...
    def eliminar_parejas(self):
        ...
        while not salir:
            ...
            if cont<len(self.cartas):
                if(carta.rango == posiblePareja.rango and (carta.palo + posiblePareja.palo) == 3):
                    print("En la mano de {0} forman par {1}  {2}".format(self.jugador, carta, pareja))
                    self.cartas.remove(carta)
                    self.cartas.remove(posiblePareja)
                print("No forman pareja")
                cont += 1
            else:
                salir = True
